...
export function StatsBar({ stats }: { stats: SearchStats }) {
  const pctUnsponsored = stats.total_count
    ? Math.round((stats.unsponsored_count / stats.total_count) * 100)
    : 0;
  const pctEligible = stats.total_count
    ? Math.round((stats.eligible_count / stats.total_count) * 100)
    : 0;
      ...
      {items.map((item) => (
        <div key={item.label} className="metric-card">
          <div className="flex items-center gap-2 mb-1">
            <item.icon size={14} style={{ color: "var(--color-brand)" }} />
            <span
          ...
          <div className="text-2xl font-semibold" style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}>
            {item.value}
          </div>
